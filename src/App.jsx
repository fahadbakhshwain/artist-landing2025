...
// مسار الأساس لصفحة GitHub
const base = import.meta.env.BASE_URL || "/";

// مسارات ملفاتك المحلية داخل public/
const paths = {
  ...
  workImages: Array.from({ length: 6 }, (_, i) => `${base}work/w${i + 1}.jpg`),
  pressImages: Array.from({ length: 8 }, (_, i) => `${base}press/p${i + 1}.jpg`),
};
...
function Hero({ onContact }) {
  const bg = `linear-gradient(rgba(0,0,0,.35), rgba(0,0,0,.35)), url('${paths.hero}')`;
  return (
  ...
  return (
    <section id="about" className="bg-white text-neutral-900">
      ...
      <div className="relative w-full h-[60vh] sm:h-[70vh] overflow-hidden">
        <img src={paths.about1} alt="Huda in studio" className="absolute inset-0 w-full h-full object-cover" loading="eager" />
      </div>
      ...
      <div className="max-w-5xl mx-auto px-6 py-16">
        ...
        <div className="space-y-5 leading-7 text-[17px]">
          <p>
            Born in 1988 in Saudi Arabia, Huda Beydoun is a multidisciplinary visual artist, photographer,
            and creative director whose work has been exhibited locally and internationally since 2009…
          </p>
          <p>
            Her distinctive visual language often pairs joyful, striking compositions with subtle emotional
            undercurrents, inviting viewers to look beyond the surface.
          </p>
          <p>
            In 2018, she moved to Paris to study fashion photography under the creative direction of Paolo
            Roversi and Dominique Issermann…
          </p>
          <p>
            Her work has been presented in galleries, cultural institutions, and art fairs… remaining rooted
      ...
      <div className="max-w-6xl mx-auto px-6 pb-24">
        <div className="grid md:grid-cols-2 gap-12 items-center mb-20">
          ...
          <div>
            <img src={paths.about2} alt="Process" className="w-full aspect-[4/3] object-cover" loading="lazy" />
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-12 items-center">
          <div className="md:order-1">
            <img src={paths.about1} alt="Concept" className="w-full aspect-[4/3] object-cover" loading="lazy" />
          </div>
...
function Work() {
  ...
  return (
    <section className="pt-24 pb-16 px-6 max-w-6xl mx-auto">
      ...
      {/* شبكة الصور (من ملفاتك المحلية مباشرة) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {paths.workImages.map((src, i) => (
          <button key={src} onClick={() => setLb({ open: true, i })} className="group text-left">
            <div className="w-full h-64 bg-neutral-100 overflow-hidden">
              <img src={src} className="w-full h-full object-cover group-hover:scale-105 transition" />
            </div>
            <div className="mt-3">
      ...
      <Lightbox
        ...
        onPrev={() => setLb(s => ({ ...s, i: (s.i - 1 + paths.workImages.length) % paths.workImages.length }))}
        onNext={() => setLb(s => ({ ...s, i: (s.i + 1) % paths.workImages.length }))}
      />
    </section>
  );
}
...
function Contact() {
  return (
    <section className="pt-24 pb-16 px-6 max-w-5xl mx-auto">
      ...
      <p className="mb-2">Email: <a href="mailto:hello@artist.com" className="underline">hello@artist.com</a></p>
